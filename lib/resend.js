...
export async function sendJobNotification(jobData) {
  const sanitizedJobData = sanitizeJobData([jobData])[0];
  ...
  for (const jobSeeker of jobSeekerList) {
    const html = `
        ...
        <p>A new job has been posted at ${sanitizedJobData.studioName}!</p>
        <p><strong>${sanitizedJobData.jobTitle}</strong></p>
        <p>Location: ${sanitizedJobData.jobLocation}</p>
        <p>For more details, click <a href="https://game-jobs.it.com/jobs/${sanitizedJobData._id}">here</a>.</p>
      `;

...
export async function sendJobNotification(jobData) {
  const sanitizeJobData = sanitizeJobData([jobData])[0];
  ...
  for (const jobSeeker of jobSeekerList) {
    const html = `
        ...
        <p>A new job has been posted at ${sanitizeJobData.studioName}!</p>
        <p><strong>${sanitizeJobData.jobTitle}</strong></p>
        <p>Location: ${sanitizeJobData.jobLocation}</p>
        <p>For more details, click <a href="https://game-jobs.it.com/jobs/${sanitizeJobData._id}">here</a>.</p>
      `;

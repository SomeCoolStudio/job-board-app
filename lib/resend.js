...
export async function sendJobNotification(jobId) {
  try {
    ...
    // Sanitize the job data by wrapping it in an array and extracting the first element.
    ...
    for (const jobSeeker of jobSeekerList) {
      const html = `
        ...
        <p>For more details, click <a href="https://game-jobs.it.com/jobs/${sanitizedData._id}" target="_blank">here</a>.</p>
      `;
